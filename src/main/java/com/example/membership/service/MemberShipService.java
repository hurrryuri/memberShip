...
public class MemberShipService implements UserDetailsService {
    ...
    //save를 이용해서 저장한다.
    //반환값은 뭘로 하지? : dto전체를 반환으로 하자
    //그런데 만약에 회원이 가입이 되어있으면?
    ...
    public MemberShipDTO saveMember(MemberShipDTO memberShipDTO){
        ...
        if(memberShip != null){ //확인했더니 이미 가입이 되어 있다면
    ...
    //로그인 //UserDetailService를 구현해서 사용한다.
    @Override
    public UserDetails loadUserByUsername(String email) throws UsernameNotFoundException {
        ...
        //try catch문으로 다른화면으로 열리던 메시지를 가지고 로그인창으로 보내던 컨트롤러 창에서 알아서 해주겠지?
        //미래의 내가
        if(memberShip == null){
